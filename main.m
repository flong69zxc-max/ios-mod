...
static void log_to_file(const char *msg) {
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath:downloads withIntermediateDirectories:YES attributes:nil error:nil];
    NSString *logPath = [downloads stringByAppendingPathComponent:@"hitbox_patch.log"];
...
static void* find_hitbox(long *offset) {
...
__attribute__((constructor)) static void init() {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 3 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        @autoreleasepool {
            ...
            for (int i = 0; i < 10; i++) {
                char buf[128];
                snprintf(buf, sizeof(buf), "  %s: %.3f", names[i], old[i]);
                log_to_file(buf);
            }
            ...
            for (int i = 0; i < 10; i++) {
                char buf[128];
                snprintf(buf, sizeof(buf), "  %s: %.3f", names[i], news[i]);
                log_to_file(buf);
            }
            
            char msg[1024];
            snprintf(msg, sizeof(msg),
                     "Адрес: %p (смещение: 0x%lX)\n\n"
                     "HEAD: %.3f → %.3f\n"
                     "TORSO_1: %.3f → %.3f\n"
                     "TORSO_2: %.3f → %.3f\n"
                     "LEGS_1: %.3f → %.3f\n"
            ...
            log_to_file("✅ Патч применён успешно");
            show_alert("✅ Патч применён", msg);
        }
    });
}
